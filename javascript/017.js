...
const getEngine = function (input, etype, ngram) {
  const opts = {}
...
const randomChunks = function (words) {
  const chunks = []

  // range: Math.floor(Math.random() * (max - min + 1)) + min;

  let tot = 0 // words.length;

  while (tot < words.length) {
    let grab = Math.floor(Math.random() * 10) + 1 // grab 1...10 words
    // if random > length of array, only grab what is remaining
    if (tot + grab > words.length) {
      grab = words.length - tot
    }
    const bite = words.slice(tot, tot + grab)
    tot += grab
